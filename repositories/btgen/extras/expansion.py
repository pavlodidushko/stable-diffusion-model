...
from modules.config import path_btgen_expansion
...
class BTGenExpansion:
    def __init__(self):
        ...
        positive_words = open(os.path.join(path_btgen_expansion, 'positive.txt'),
        ...
        print(f'BTGen V2 Expansion: Vocab with {len(debug_list)} words.')
        ...
        self.model = AutoModelForCausalLM.from_pretrained(path_btgen_expansion)
        self.model.eval()
        ...
        print(f'BTGen Expansion engine loaded for {load_device}, use_fp16 = {use_fp16}.')
    ...
    def __call__(self, prompt, seed):
        ...
        if self.patcher.current_device != self.patcher.load_device:
            print('BTGen Expansion loaded by itself.')
